...
$last_planet_num = 9; //元は16        //add a planet
...
for ($i = 0; $i <= $last_planet_num; $i++) {
  drawboldtext($im, 18, 0, $margins + $i  * $cell_width, $cell_height, $red, dirname(__FILE__) . '/HamburgSymbols.ttf', chr($pl_glyph[$i]), 0);    //across the top

  // display planet data in the right-hand table
  drawboldtext($im, 16, 0, -265 + $left_margin_planet_table, $cell_height + $cell_height * ($i + 1), $black, dirname(__FILE__) . '/HamburgSymbols.ttf', chr($pl_glyph[$i]), 0);
  imagettftext($im, 10, 0, $margins + $left_margin_planet_table + $cell_width * 2, $cell_height + $cell_height * ($i + 1) - 3, $blue, dirname(__FILE__) . '/arial.ttf', $pl_name[$i]);

  $sign_num1 = floor($longitude1[$i] / 30) + 1;
  drawboldtext($im, 14, 0, $margins + $left_margin_planet_table + $cell_width * 5, $cell_height + $cell_height * ($i + 1), $black, dirname(__FILE__) . '/HamburgSymbols.ttf', chr($sign_glyph[$sign_num1]), 0);
  imagettftext($im, 10, 0, $margins + $left_margin_planet_table + $cell_width * 6, $cell_height + $cell_height * ($i + 1) - 3, $blue, dirname(__FILE__) . '/arial.ttf', Convert_Longitude($longitude1[$i]) . " " . $rx1[$i]);

  $sign_num2 = floor($longitude2[$i] / 30) + 1;
  drawboldtext($im, 14, 0, $margins + $left_margin_planet_table + $cell_width * 10, $cell_height + $cell_height * ($i + 1), $red, dirname(__FILE__) . '/HamburgSymbols.ttf', chr($sign_glyph[$sign_num2]), 0);
  imagettftext($im, 10, 0, $margins + $left_margin_planet_table + $cell_width * 11, $cell_height + $cell_height * ($i + 1) - 3, $blue, dirname(__FILE__) . '/arial.ttf', Convert_Longitude($longitude2[$i]) . " " . $rx2[$i]);
}
...
for ($i = 0; $i <= $last_planet_num; $i++) {
  for ($j = 0; $j <= $last_planet_num; $j++) {
    ...
    if ($q > 0) {
      drawboldtext($im, 14, 0, $margins + $cell_width * ($i + 0.20), $cell_height + $cell_height * ($j + 1 - 0.20), $asp_color[$q], dirname(__FILE__) . '/HamburgSymbols.ttf', chr($asp_glyph[$q]), 0);
    }
  }
}
