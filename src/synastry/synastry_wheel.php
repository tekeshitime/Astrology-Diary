...
$copyright2 = "and generated at www.astrowin.org";

$line1 = safeEscapeString($_GET["l1"]);
$line2 = safeEscapeString($_GET["l2"]);

$retrograde1 = safeEscapeString($_GET["rx1"]);
...
if (get_magic_quotes_gpc()) {
  $longitude1 = unserialize(stripslashes($_GET["p2"]));
  //    $hc1 = unserialize(stripslashes($_GET["hc1"]));
  //    $house_pos1 = unserialize(stripslashes($_GET["hpos"]));

  $longitude2 = unserialize(stripslashes($_GET["p1"]));
  //    $hc2 = unserialize(stripslashes($_GET["hc2"]));
  //    $house_pos2_in_1 = unserialize(stripslashes($_GET["hpos2"]));
} else {
  $longitude1 = unserialize($_GET["p2"]);
  //    $hc1 = unserialize($_GET["hc1"]);
  //    $house_pos1 = unserialize($_GET["hpos"]);

  $longitude2 = unserialize($_GET["p1"]);
  //    $hc2 = unserialize($_GET["hc2"]);
  //    $house_pos2_in_1 = unserialize($_GET["hpos2"]);
}

$hc1 = $_SESSION['hc1'];
$house_pos1 = $_SESSION['house_pos1'];
$hc2 = $_SESSION['hc2'];
$house_pos2_in_1 = $_SESSION['house_pos2_in_1'];
...
// specific colors
$planet_color = $red;   //was $cyan;
$planet_color_2 = $black;
...
$last_planet_num = 7 + 2;        //add a planet
...
imagefilledrectangle($im, 0, 0, $size_of_rect, $size_of_rect + $y_top_margin, $white);

// MUST BE HERE - I DO NOT KNOW WHY - MAYBE TO PRIME THE PUMP
imagettftext($im, 10, 0, 0, 0, $black, dirname(__FILE__) . '/arial.ttf', " ");    //arial.ttf
...
//data for chart
imagettftext($im, 8, 0, 10, 20, $black, dirname(__FILE__) . '/arial.ttf', $line1);
imagettftext($im, 8, 0, 10, 38, $black, dirname(__FILE__) . '/arial.ttf', $line2);

imagettftext($im, 8, 0, 620, 828, $black, dirname(__FILE__) . '/arial.ttf', $copyright1);
imagettftext($im, 8, 0, 620, 841, $black, dirname(__FILE__) . '/arial.ttf', $copyright2);
...
for ($i = 1; $i <= 12; $i = $i + 1) {
  ...
  imagettftext($im, 14, 0, $xy[0] + $center_pt_x, $xy[1] + $center_pt_y, $clr_to_use, dirname(__FILE__) . '/HamburgSymbols.ttf', chr($sign_glyph[$sign_pos]));
  ...
  imagettftext($im, 10, 0, $xy[0] + $center_pt_x, $xy[1] + $center_pt_y, $black, dirname(__FILE__) . '/arial.ttf', $t . chr(176));
  ...
  imagettftext($im, 10, 0, $xy[0] + $center_pt_x, $xy[1] + $center_pt_y, $black, dirname(__FILE__) . '/arial.ttf', $t . chr(39));
}
...
for ($i = 1; $i <= 12; $i = $i + 1) {
  ...
  imagettftext($im, 10, 0, $xy[0] + $center_pt_x, $xy[1] + $center_pt_y, $black, dirname(__FILE__) . '/arial.ttf', $i);
...
for ($i = 1; $i <= 12; $i++) {
  ...
  drawboldtext($im, 12, 0, $x1 + $center_pt_x, $y1 + $center_pt_y, $clr_to_use, dirname(__FILE__) . '/HamburgSymbols.ttf', chr($sign_glyph[$i]), 0);  //used to be boldness of 1 - last number
}
...
for ($i = $num_planets - 1 - 2; $i >= 0; $i--) {
  ...
  imagettftext($im, 16, 0, $xy[0] + $center_pt_x, $xy[1] + $center_pt_y, $planet_color, dirname(__FILE__) . '/HamburgSymbols.ttf', chr($pl_glyph[$sort_pos[$i]]));
  ...
  imagettftext($im, 10, 0, $xy[0] + $center_pt_x, $xy[1] + $center_pt_y, $clr_to_use, dirname(__FILE__) . '/HamburgSymbols.ttf', chr($sign_glyph[$sign_pos]));
  ...
  if (strtoupper(mid($retrograde1, $sort_pos[$i] + 1, 1)) == "R") {
    ...
    imagettftext($im, 10, 0, $xy[0] + $center_pt_x, $xy[1] + $center_pt_y, $red, dirname(__FILE__) . '/HamburgSymbols.ttf', chr(62));
  }
}
...
for ($i = $num_planets - 1; $i >= 0; $i--) {
  ...
  imagettftext($im, 16, 0, $xy[0] + $center_pt_x, $xy[1] + $center_pt_y, $planet_color_2, dirname(__FILE__) . '/HamburgSymbols.ttf', chr($pl_glyph[$sort_pos[$i]]));
  ...
  imagettftext($im, 10, 0, $xy[0] + $center_pt_x, $xy[1] + $center_pt_y, $planet_color_2, dirname(__FILE__) . '/arial.ttf', $t . chr(176));
  ...
  imagettftext($im, 10, 0, $xy[0] + $center_pt_x, $xy[1] + $center_pt_y, $planet_color_2, dirname(__FILE__) . '/arial.ttf', $t . chr(39));
