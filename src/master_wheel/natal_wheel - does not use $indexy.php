<?php
  ...
  $Ascendant = $hc[1];
  $hc[13] = $hc[1];             //without this, PhpED craskes at line 386 (when doing Venus in my chart)
  ...
  $dist_from_diameter1a = 12 * 2;             //distance inner planet glyph is from circumference of wheel - for line
  $dist_from_diameter2 = 58 * 2;              //distance outer planet glyph is from circumference of wheel
  $dist_from_diameter2a = 28 * 2;             //distance outer planet glyph is from circumference of wheel - for line
...
// ------------------------------------------
  ...
  for ($i = 1; $i <= 12; $i = $i + 1)
  {
    ...
    //imagettftext($im, 28, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $clr_to_use, dirname(__FILE__) . '/HamburgSymbols.ttf', chr($sign_glyph[$sign_pos]));
    imagettftext($im, 28, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $clr_to_use, dirname(__FILE__) . '/ASTRC___.TTF', chr($sign_glyph[$sign_pos]));
    ...
    imagettftext($im, 20, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $black, dirname(__FILE__) . '/arial.ttf', $t . chr(176));    //house cusp degree
    ...
    imagettftext($im, 20, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $black, dirname(__FILE__) . '/arial.ttf', $t . chr(39));
  }
...
// ------------------------------------------
  ...
  for ($i = 1; $i <= 12; $i = $i + 1)
  {
    ...
    imagettftext($im, 20, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $black, dirname(__FILE__) . '/arial.ttf', $i);
  }
...
if ($in_test == 0)
{
  ...
  for ($i = $num_planets - 1; $i >= 0; $i--)
  {
    ...
    if ($temp != $house_num)
    {
      $planets_done = 1;        //this planet is in a different house than the last one - this planet is the first one in this house, in other words
    }
    ...
    {
      imagettftext($im, 32, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $planet_color, dirname(__FILE__) . '/ASTRC___.TTF', chr($pl_glyph[$sort_pos[$i]]));
    }
    else
    {
      imagettftext($im, 32, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $planet_color, dirname(__FILE__) . '/HamburgSymbols.ttf', chr($pl_glyph[$sort_pos[$i]]));
    }
    ...
    imagettftext($im, 18, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $planet_color, dirname(__FILE__) . '/arial.ttf', $t . chr(176));
    ...
    //imagettftext($im, 20, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $clr_to_use, dirname(__FILE__) . '/HamburgSymbols.ttf', chr($sign_glyph[$sign_pos]));
    imagettftext($im, 20, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $clr_to_use, dirname(__FILE__) . '/ASTRC___.TTF', chr($sign_glyph[$sign_pos]));
    ...
    imagettftext($im, 16, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $planet_color, dirname(__FILE__) . '/arial.ttf', $t . chr(39));
    ...
    if (strtoupper(mid($retrograde, $sort_pos[$i] + 1, 1)) == "R")
    {
      ...
      imagettftext($im, 20, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $red, dirname(__FILE__) . '/HamburgSymbols.ttf', chr(62));
    }
  }
...
else
{
  ...
  for ($i = 0; $i <= 359.99; $i = $i + $step)
  {
    ...
    imagettftext($im, 32, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $planet_color, dirname(__FILE__) . '/ASTRC___.TTF', chr($pl_glyph[$glyph]));
    ...
    //imagettftext($im, 12, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $deg_min_color, dirname(__FILE__) . '/arial.ttf', $t);
...
Function display_house_number($num, $angle, $radii, &$xy)
{
  ...
  if ($num == 1)
  {
    ...
    $ypos0 = -$half_char_height * 0.25;
    $y_adj = sin(deg2rad($angle)) * $char_height;
  }
  elseif ($num == 2)
  {
    $xpos0 = -$char_width * 1.50;
